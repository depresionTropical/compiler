...
class ErrorTable:
    ...
    def add_error(self, n:int,line:int, lex:str, messag:str):
        # Lógica para agregar un error a la tabla
        self.error_table.append(
            {
                "Token de error": f'ES{n}',
                "Línea":line,
                "Lexema": lex,
                "Descripción": messag
            }
    ...
    def check_operation(self):
        ...
        for line in self.source_code:
            ...
            if line.count('=') > 0 and (line.count('+') >0 or line.count('-') > 0 or line.count('/') > 0) :
                # for lex in line:
                    # print(lex)
                self.check_error(n,line,num_line)

    def check_error(self,n:int,line:list[str],num_line:int):
        print(line)
        index_eq = line.index('=')
        ...
        if result == 'numero' and (left != 'numero' or right != 'numero'):
            ...
            # print(erro_token)
            self.add_error(n,num_line,erro_token,'Error de tipo incompatiblidad de numero')
        # float
